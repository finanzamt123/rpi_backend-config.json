...
TARGET_EC = 500     # ppm Sollwert
WAIT_MIN  = 30      # Minuten zwischen Korrekturen
...
def sensor_data():
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO sensor_data(water_temp, tds_ppm) VALUES(?, ?)",
...
def next_pump():
    ...
    if (now - last_correction).total_seconds() < WAIT_MIN*60:
        return '0'
    conn = get_db()
    cur = conn.execute("SELECT tds_ppm FROM sensor_data ORDER BY id DESC LIMIT 1")
    row = cur.fetchone()
    conn.close()
    current = row[0] if row else TARGET_EC
    err = TARGET_EC - current
    seconds = max(0, min(10, int(err/50)))  # einfacher P-Regler
    if seconds > 0:
        last_correction = now
    return str(seconds)
...
def graph24h():
    ...
    ax.legend()
    ax.set_xlabel('Zeit')
    ax.set_title('Letzte 24h')
    os.makedirs(IMAGE_DIR, exist_ok=True)
    path = os.path.join(IMAGE_DIR, 'last24h.png')
    fig.savefig(path)
    plt.close(fig)
...
def daily_archive():
    ...
    filename = os.path.join(ARCHIVE_DIR, datetime.now().strftime('%Y-%m-%d')+'.csv')
    df.to_csv(filename, index=False)

sched = BackgroundScheduler()
sched.add_job(daily_archive, 'cron', hour=23, minute=59)
sched.start()
